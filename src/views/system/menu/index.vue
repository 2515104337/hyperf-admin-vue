...
<script setup lang="ts">
  ...
  const parentMenuId = ref<number | null>(null) // 新增子菜单时的父级ID
  ...
  const getMenuList = async (): Promise<void> => {
    ...
    try {
      const list = await fetchGetAllMenuList()
  ...
  const { columnChecks, columns } = useTableColumns(() => [
    ...
    },
    {
      ...
      label: '状态',
      formatter: (row: AppRouteRecord) => {
        const isEnable = row.meta?.isEnable !== false
  ...
  const handleAddSubMenu = (row: AppRouteRecord): void => {
    dialogType.value = 'menu'
    editData.value = null
    parentMenuId.value = row.id || null // 设置父级ID
    ...
    try {
      if (formData.id) {
        ...
        if (parentMenuId.value) {
          formData.parentId = parentMenuId.value
        }
